...
ffmpeg_decoder::ffmpeg_decoder(std::string path)
{
    this->path = path;
}


void ffmpeg_decoder::intialize()
{
...
void ffmpeg_decoder::setOnframeCallbackFunction(std::function<void(uint8_t *)> func)
{
    onFrame = func;
}


void ffmpeg_decoder::playMedia()
{
    AVPacket packet;
    AVFrame * pFrame;
